...
void CPeripheral::RegisterKeyboardHandler(KEYBOARD::IKeyboardInputHandler* handler, bool bPromiscuous)
{
  ...
  {
    CAddonInputHandling* addonInput = new CAddonInputHandling(m_manager, this, handler);
    RegisterKeyboardDriverHandler(addonInput, bPromiscuous);
    m_keyboardHandlers[handler].reset(addonInput);
  }
}
